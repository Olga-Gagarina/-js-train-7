...
function createSymbolWithDescription(property) {
  const symbolWithDescription = Symbol(property);
...
function createAndCompareTwoSymbols(value) {
  let holica = Symbol ( "sadok vushnevuy kolo hatu");
  let caliho = Symbol ( "sadok vushnevuy kolo hatu");
  return holica === caliho;
...
function createSymbolForKey(key) {
  let symbolForKey = Symbol.for(key) ;
  return symbolForKey;
...
let myObject = {
  ...
  next() {
    if( this.currentIndex < this.category.length) { 
       const { name} = this.category[this.currentIndex];
    this.currentIndex++;
    return{
